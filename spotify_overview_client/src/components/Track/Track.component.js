...
import { format_duration, get_year, parse_pitch_class, catch_errors } from '../../utils/utils';
import { get_track_info } from '../../utils/spotify_utils';
...
class Track extends Component {
  ...
  async getData() {
    ...
    const { track, audioAnalysis, audioFeatures } = await get_track_info(trackId);
          ...
          <Main>
            <TrackContainer>
              ...
              <Info>
                ...
                <Album>
                  ...
                  &middot; {get_year(track.album.release_date)}
                </Album>
                <PlayTrackButton
            ...
            {audioFeatures && audioAnalysis && (
              <AudioFeatures>
                <Features>
                  <Feature>
                    <FeatureText>{format_duration(audioFeatures.duration_ms)}</FeatureText>
                    <FeatureLabel>Duration</FeatureLabel>
                  </Feature>
                  <Feature>
                    <FeatureText>{parse_pitch_class(audioFeatures.key)}</FeatureText>
